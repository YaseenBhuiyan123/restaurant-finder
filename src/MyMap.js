...
import {Radio, RadioGroup, FormControlLabel, List, ListItem, ListItemText} from "@material-ui/core";
import "./MyMap.css";
const google_key = process.env.REACT_APP_google_key;
const mapbox_key = process.env.REACT_APP_mapbox_key;

export default function MyMap( { latitude, longitude, type, keyword, distance } ) {
  ...
  const getDirections = () => {
    ...
    +selectedMarker.geometry.location.lat+","
    +selectedMarker.geometry.location.lng
    +",17z/data=!4m4!4m3!1m0!1m1!4e1", '_blank').focus();
  }
        ...
        {places.map(place => {
          return (
            <ListItem key={place.place_id}>
              <ListItemText 
                primary={place.name} 
                secondary={"Rating: "+place.rating+", Price Level: "+place.price_level}
              />
            </ListItem>
          )
        })}
